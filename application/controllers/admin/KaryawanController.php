<?php

use application\controllers\AdminMainController;

class KaryawanController extends AdminMainController
{
    function __construct()
    {
        parent::__construct();
        $this->model('karyawan');
    }
    public function index()
    {
        $data = $this->karyawan->showJoinKaryawan();
        $this->template('admin/karyawan/index', 'Data Karyawan', $data);
    }

    public function edit($id)
    {
        $karyawan = $this->karyawan->show($id);
        $user_id = $karyawan[0]['user_id'];
        $this->model('jabatan');
        $jabatan = $this->jabatan->showWhere(array('user_id' => $user_id));
        $data = ['data' => $karyawan[0], 'jabatan' => $jabatan];
        $this->template('admin/karyawan/edit', 'Edit Data Karyawan', $data);
    }

    public function update()
    {
        $error = array();
        $data = array();
        $user_id = $_POST['user_id'];
        $data['nama_karyawan'] = $_POST['nama_karyawan'];
        $data['nik'] = $_POST['nik'];
        $data['jenis_kelamin'] = $_POST['jenis_kelamin'];
        $data['jabatan_id'] = $_POST['jabatan_id'];
        $data['tanggal_masuk'] = $_POST['tanggal_masuk'];
        $data['status'] = $_POST['status'];

        if ($_FILES['foto']['name'] != null) {
            $valid_extensi = array('png', 'jpg');
            $x = explode('.', $_FILES['foto']['name']);
            $extensi = strtolower(end($x));
            $photoName = 'karyawan_' . $user_id . '_' . time() . "." . $extensi;
            $size = $_FILES['foto']['size'];
            $file_tmp = $_FILES['foto']['tmp_name'];
            if (in_array($extensi, $valid_extensi) == false) array_push($error, "Ektensi Gambar harus PNG atau JPG");
            if ($size > 1044070) array_push($error, "Ukuran Gambar Terlalu Besar, Harus dibawah 1Mb");

            $data['photo'] = $photoName;
        }

        $id = $_POST['id_karyawan'];

        if (count($error) == 0) {
            $simpan = $this->karyawan->update($data, $id);
            if ($simpan) :
                move_uploaded_file($file_tmp, $_SERVER['DOCUMENT_ROOT'] . '/penggajihan/public/img/' . $photoName);
                $this->redirect('admin/karyawan');
            endif;
        } else {
            $karyawan = $this->karyawan->show($id);
            $user_id = $karyawan[0]['user_id'];
            $this->model('jabatan');
            $jabatan = $this->jabatan->showWhere(array('user_id' => $user_id));
            $data = ['data' => $karyawan[0], 'jabatan' => $jabatan];
            $this->template('admin/karyawan/edit', 'Edit Data Karyawan', array('msg' => $error, 'data' => $data));
        }
    }

    public function delete($id)
    {
        $hapus = $this->karyawan->delete($id);
        if ($hapus) $this->redirect('admin/karyawan');
    }
}
